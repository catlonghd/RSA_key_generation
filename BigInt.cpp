...
void BigInt::subBigInt(BigInt& a, BigInt& b)
{
...
void BigInt::multBigInt(BigInt& a, BigInt& b)
{
...
void BigInt::copyBigInt(BigInt& a, int index)
{
    for (int i = 0; i < nSize;i++)
    {
...
void gcdBigInt(BigInt& a, BigInt& b, BigInt& result)
{
    ...
    if (tmpb.msbBigInt() == 0 && tmpb.digit[0] == 0)
    {
    ...
    else
    {
        BigInt q(a.nSize);
        BigInt r(a.nSize);
        divBigInt(tmpa, tmpb, q, r);
        gcdBigInt(tmpb, r, result);
    }
}
...
void BigInt::expoModNBigInt(BigInt& x, BigInt& y, BigInt& N, BigInt& result)
{
    ...
    else
    {
        ...
        BigInt value2(nSize);
        value2.digit[0] = 2;
        divBigInt(y, value2, *this, reminder);

        temp.copyBigInt(*this, 0);
        expoModNBigInt(x, temp, N, result);

        multBigInt(result, result);
